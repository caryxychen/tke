...
func (r *REST) Delete(ctx context.Context, name string, deleteValidation rest.ValidateObjectFunc, options *metav1.DeleteOptions) (runtime.Object, bool, error) {
	...
	if options.Preconditions.UID == nil {
		options.Preconditions.UID = &pol.UID
	} else if *options.Preconditions.UID != pol.UID {
		err = apierrors.NewConflict(
			authz.Resource("Policies"),
			name,
			fmt.Errorf("precondition failed: UID in precondition: %v, UID in object meta: %v", *options.Preconditions.UID, pol.UID),
		)
		return nil, false, err
	}

	// upon first request to delete, we switch the phase to start pol termination
	if pol.DeletionTimestamp.IsZero() {
		...
		err = r.Store.Storage.GuaranteedUpdate(
			...
			storage.SimpleUpdate(func(existing runtime.Object) (runtime.Object, error) {
				existingBinding, ok := existing.(*authz.Policy)
				if !ok {
					// wrong type
		...
		if err != nil {
			err = storageerr.InterpretGetError(err, authz.Resource("Policies"), name)
			err = storageerr.InterpretUpdateError(err, authz.Resource("Policies"), name)
	...
	if len(pol.Finalizers) != 0 {
		err = apierrors.NewConflict(authz.Resource("Policies"), pol.Name, fmt.Errorf("the system is ensuring all content is removed from this pol.  Upon completion, this pol will automatically be purged by the system"))
		return nil, false, err
	}
